    ...
    public class ArticleController: ControllerBase
    {
        ...
        public void ExecuteSaveCommand(object o)
        {
                ...
                _viewModel.SelectedModel.Description is null)
            {
                ...
                return;
            } else if (_viewModel.SelectedModel.Price <= 0)
            {
                var _popupWindowController = _app.Container.Resolve<PopupWindowController>();
                _popupWindowController.DisplayText("Please make sure to fill in a positive number as the price!");
            ...
            if (_viewModel.NewItem)
            {
                ...
                {
                    var _popupWindowController = _app.Container.Resolve<PopupWindowController>();
                    _popupWindowController.DisplayText("The data coundn't be saved. Please make sure that the article number is unique!");
                    return;
                }
                LoadNewData();
        ...
        public void ExecuteDeleteCommand(object o)
        {
            ...
            {
                var _popupWindowController = _app.Container.Resolve<PopupWindowController>();
                _popupWindowController.DisplayText("An error occured and the data couldn't be deleted. Please make sure that the article is not related to an order!");
                return;
            }
