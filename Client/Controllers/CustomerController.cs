...
namespace Baka.Hipster.Burger.Client.Controllers
{
    public class CustomerController : ControllerBase
    {
        ...
        public void ExecuteSaveCommand(object o)
        {
            ...
            else if (_viewModel.SelectedModel.PostalCode <= 0)
            {
                var _popupWindowController = _app.Container.Resolve<PopupWindowController>();
                _popupWindowController.DisplayText("Please make sure that the postcode number is a positive number!");
                ...
                if (_viewModel.NewItem)
            {
                ...
                {
                    var _popupWindowController = _app.Container.Resolve<PopupWindowController>();
                    _popupWindowController.DisplayText("The data coundn't be saved. Please make sure that the phone number is unique!");
                    return;
                }
                LoadNewData();
        ...
        public void ExecuteDeleteCommand(object o)
        {
            ...
            {
                var _popupWindowController = _app.Container.Resolve<PopupWindowController>();
                _popupWindowController.DisplayText("An error occured and the data couldn't be deleted. Please make sure that the customer is not related to an order!");
                return;
            }
